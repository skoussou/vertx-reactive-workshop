...
----
      ...
      ** Expose a *POST* operation on endpoint *_/homeplan/{id}_* to be handled by the method *_addOne_* (provided for you in the following code snipet) registering a single HomePlan. HINT: http://vertx.io/docs/vertx-web/java/#_routing_by_http_method[Routing by HTTP method]
