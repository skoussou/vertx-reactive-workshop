...
* Use resources at http://vertx.io/docs/vertx-web/java/[Vert.X Web] to create an HTTP server so that it exposes the services in port 8181 in the *_start()_* method of the MainVerticle.java
...
* how to start multiple Verticle Instances and what it means from HTTP *(Launcher manages that ... ned to find how in pom.xml). Also show that in starting anothe Verticle)*
