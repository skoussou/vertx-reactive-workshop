...
In this lab we further enhance our applications so that a new *_sensor-generator_* Vert.x application is added to generate for a registered homeplan temperature in each of the rooms (sensorLocations) registered, taking into account the previous temperature and if an AIRCON is on/off and if it is set to INCREASE/DECREASE the room temeperature. *_device-management_* offers a service SEND/REPLY to read a specific device status available as a SEND/REPLY EventBus service.
...
* Create Content for verticles in *_sensor-generator_* maven project to complete the service
  ...
  ** Using resources at link:http://vertx.io/docs/vertx-core/java/#_the_theory[Vert.x EventBus] around SEND/REPLY pattern Fix method *_getHomePlan()_* to send the given homeplan id to address *_#homeplan_*
  ...
  ** Using resources at link:http://vertx.io/docs/vertx-core/java/#_the_theory[Vert.x EventBus] around SEND/REPLY pattern Fix method *_getDeviceStatus()_* to send the given sensor id to address *_#device-data_*
