...
* http://vertx.io/docs/vertx-core/java/#event_bus[The EventBus] sending/consuming messages over a clustered bus (SEND/REPLY)
* *(TBC)* REACTIVE JAVA via handlers
* *(TBC)* REACTIVE JAVA via Futures??
* HTTP Rest Client
...
open new terminal
cd [REPOSITORY CLONED DIR - Branch LAB-2]/sensor-generator
mvn compile vertx:run -Dvertx.runArgs="-cluster -Djava.net.preferIPv4Stack=true"
----
...
ORGANIZERS PART
clone/unzip "ReactiveVertX Branch - LAB 2"
which will have solution of LAB 2
Sensor Generator Application with all algorithms ready for sensor generation in utility methods including traversing through the list of HomePlan 
                         location-sensors and list of DevicesStatus and with the calling of the REST homeplan in place WITH INSTRUCTIONs (To be erviewed). Will have 
clustering in place OOB
Device Management Application with solutoin from LAB 2

PARTICIPANTS PART
   Create Content for verticles in Sensor Generator
        - Implement Rest Client to get HomePlan IDs (from GET /homeplan) localhost:8080 --> implementaion of getHomePlanIds
        - send: Messages to #homeplans to retrieve Homeplan Info and handle response --> implementation of getHomePlan
       - send: Messages to #device-data to retrieve current device status
       WE WILL Provide algorithm to combine the 2 above call REPLIES content to create the message for #ambience-data 
       - Call publish (implemented by US as DUMMY) logging the messages generated

   Create Content for verticles in HomePlan
    - Implement consume/reply from #homeplans

   Create Content for verticles in Device Management
    - consume/reply: Messages from #device-data on the bus, call a method 
    - to get data from shared-data (THIS PART WE WILL PROVIDE) and PARTICIPANTS will return response to reply (SEND/REPLY pattern)
